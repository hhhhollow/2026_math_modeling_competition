"""
Q3.3  公平对比：当前规律（12年地平线） vs 最优规律（12年地平线）
  注：Q2 寿命表用了 10 年地平线，对 A2 这类"边缘退役"台低估了真实寿命。
  本步统一到 12 年地平线（覆盖正常退役设备最长寿命 A2 11.57y），EAC 比较才有意义。
"""
import pandas as pd
import numpy as np
...
cur = pd.read_csv(ROOT / "Q3输出/tables/current_rule_20y.csv")  # 文件名兼容，内容为 12y 地平线
...
print(f"\n10 台合计 EAC:")
print(f"  当前规律 (12y):     {total_cur:.2f} 万元/年")
...
print(f"\n12 年内退役的 {len(retire_now)} 台: EAC 节省 "
      f"{(retire_now['EAC_cur'].sum()-retire_now['EAC_opt'].sum()):.1f} 万元/年")
print(f"12 年内不退役的 {len(noretire_now)} 台 (EAC 为上限估计): "
