...
\medskip
\noindent
\textbf{问题二}：从附件 2 抽取每台经验维护周期 $(T_M^i, T_L^i)$，其中 A4、A8 历史 0 次大维护，与题目修订后约束 $0 \le n_l^i \le 4$ 次/年相符。在 $\gamma_q$ 共线性问题处用网格交叉验证确认 $\gamma = 0$ 最优，进一步比较线性 vs 指数衰减（验证 RMSE $12.96 < 13.59$）确定线性模型，并在训练+验证段重拟合最终系数。向前模拟 10 年，以"滚动 365 日均值首次跌破 37 且大维护改善不足"判定寿命，得 7 台 10 年内退役（按先后：\textbf{A10}: 1.00y、A8: 1.80y、A5: 1.87y、A9: 2.97y、A7: 3.49y、A3: 4.28y、A1: 4.46y），其余 3 台不退役。

\medskip
\noindent
\textbf{问题三}：定义等值年化成本 $\mathrm{EAC} = (c_{\text{buy}} + N_M c_m + N_L c_l)/L$（其中 $N_M, N_L$ 为寿命 $L$ 内的维护总次数；$L$ 截断上限取 12 年，覆盖正常退役设备最长寿命 A2 的 11.57 年）。在网格 $T_M \in \{30,...,120\}$、$T_L \in \{120,...,720, \infty\}$ 上扫描 810 个组合，找最小 EAC。结果显示 \textbf{10 台合计可节省 189.9 万元/年（13.3\%）}，其中 A2 节省 62.7\%、A6 节省 58.5\% 最显著；A5、A8、A10 因寿命太短，最优 EAC 仍超 170 万元/年，建议优先考虑提前更换或单独复核。

\medskip
\noindent
\textbf{问题四}：对 $c_{\text{buy}}, c_m, c_l$ 在 $\pm 30\%$ 范围扫描，全队 EAC 弹性 $\varepsilon_{c_{\text{buy}}} = 0.8293 \gg \varepsilon_{c_m} = 0.1067 > \varepsilon_{c_l} = 0.0637$，\textbf{设备购置费是核心成本驱动}。最优维护规律 30\% 完全稳定（A2, A4, A6），其余 7 台在部分成本场景下切换。三参数同向 $\pm 30\%$ 时全队 EAC 在 \textbf{[869.1, 1614.1] 万元/年} 区间。
...
$T_M \in \{30, 40, 50, 60, 70, 80, 90, 100, 120\}$（9 档），$T_L \in \{120, 180, 240, 300, 360, 450, 540, 720, \infty\}$（9 档），每台 81 组合 $\times$ 10 台 = 810 次 12 年地平线仿真（H=4383 天，覆盖正常退役设备最长寿命 A2 11.57 年），总耗时约 3.5 秒。
