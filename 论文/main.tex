...
\medskip
\noindent
\textbf{问题二}：从附件 2 抽取每台经验维护周期 $(T_M^i, T_L^i)$，其中 A4、A8 历史 0 次大维护，与题目修订后约束 $0 \le n_l^i \le 4$ 次/年相符。在 $\gamma_q$ 共线性问题处用网格交叉验证确认 $\gamma = 0$ 最优，进一步比较线性 vs 指数衰减（验证 RMSE $12.96 < 13.59$）确定线性模型，并在训练+验证段重拟合最终系数。向前模拟 10 年，以"滚动 365 日均值首次跌破 37" 判定寿命，得 7 台 10 年内退役（按先后：\textbf{A10}: 1.00y、A8: 1.80y、A5: 1.87y、A9: 2.97y、A7: 3.49y、A3: 4.28y、A1: 4.46y），其余 3 台不退役。

\medskip
\noindent
\textbf{问题三}：定义等值年化成本 $\mathrm{EAC} = (c_{\text{buy}} + N_M c_m + N_L c_l)/L$（其中 $N_M, N_L$ 为寿命 $L$ 内的维护总次数）。在网格 $T_M \in \{30,...,120\}$、$T_L \in \{120,...,720, \infty\}$ 上扫描 810 个组合，找最小 EAC。结果显示 \textbf{10 台合计可节省 184.9 万元/年（13.1\%）}，其中 A6 节省 66.6\%、A2 节省 61.1\% 最显著；A5、A8、A10 因寿命太短，最优 EAC 仍超 170 万元/年，建议优先考虑提前更换或单独复核。

\medskip
\noindent
\textbf{问题四}：对 $c_{\text{buy}}, c_m, c_l$ 在 $\pm 30\%$ 范围扫描，全队 EAC 弹性 $\varepsilon_{c_{\text{buy}}} = 0.8265 \gg \varepsilon_{c_m} = 0.1085 > \varepsilon_{c_l} = 0.0648$，\textbf{设备购置费是核心成本驱动}。最优维护规律 30\% 完全稳定（A2, A4, A6），其余 7 台在部分成本场景下切换。三参数同向 $\pm 30\%$ 时全队 EAC 在 \textbf{[855.1, 1588.1] 万元/年} 区间。
...
A4、A8 历史 0 次大维护，本文按 "复现工厂当前策略" 的原则取 $T_L^i = \infty$。读者可能质疑："2 年观测 0 次大维护就预测 10 年都不做大维护，是否过激？" 为此对 $T_L^i \in \{\infty, 262.5, 365, 180\}$ 四种情形做敏感性核查（详见附录 \texttt{Q2输出/code/step7\_robust\_TL\_check.py}）：
...
\begin{tabular}{ccccc}
\toprule
$i$ & $T_L = \infty$（现行） & $T_L = 262.5$（fallback） & $T_L = 365$（每年 1 次） & $T_L = 180$（每年 2 次） \\
\midrule
...
\textbf{结论}：A4 因 $\beta_4 > 0$（启动期外推伪影），$T_L$ 选择对寿命毫无影响；A8 在最激进假设（$T_L = 180$，每年 2 次大维护）下寿命延长 0.44 年（约 +24\%），但仍属短寿命高风险台。\textbf{$T_L = \infty$ 是最忠实复现工厂历史策略，有限 $T_L$ 可作为 A8 的工程复核方案。}
...
\textbf{合计节省 184.9 万元/年（13.1\%）}。详见图~\ref{fig:Q3_comp}。
...
\midrule
$c_{\text{buy}}$ & \textbf{0.8265} & 914.72 & 1221.60 & 1519.58 \\
